...
var GLOBAL_SPACING = 64;
var CARD_WIDTH = 190;
var CARD_HEIGHT = CARD_WIDTH * 9 / 16;
var CARD_MARGIN = 16;
var ACTIVE_CARD_MULTIPLIER = 1.5;
var INITIAL_ACTIVE_CARD_POSITION = 0;
var userScroll = true;
...
function getCardPosition(card) {
  for (var position = 0; position < getCards().length; position++) {
    if (getCardId(getCardByPosition(position)) == getCardId(card)) return position;
  }
...
function decreaseActiveCard() {
  ...
  if (position != getCardPosition(getActiveCard())) setActiveCard(getCardByPosition(position));
}

function increaseActiveCard() {
  var position = getCardPosition(getActiveCard()) + 1;
  if (position > getCards().length - 1) position = getCards().length - 1;
  if (position != getCardPosition(getActiveCard())) setActiveCard(getCardByPosition(position));
} // TODO
...
function init() {
  ...
  cards.forEach(function (card, position) {
    ...
    if (position != INITIAL_ACTIVE_CARD_POSITION) background.style.display = "none";
...
function handler() {
  ...
  var scrolling;
  getCardsWrap().addEventListener("scroll", function () {
    ...
    scrolling = setTimeout(function () {
      if (!userScroll) {
        ...
        if (position != getCardPosition(getActiveCard())) setActiveCard(getCardByPosition(Math.round(position)));
      }
    }, 100);
  });
  getCards().forEach(function (card) {
    card.addEventListener("click", function () {
      if (card != getActiveCard()) setActiveCard(card);
    });
